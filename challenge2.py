...
# here the algorithm will learn all the features for each hashtag. For all hashtags get feature vectors and save them
# in a list. also in another list will be all the scores (funny/not funny)
# then we will do feature extraction for all the test hastags and put the feature vectors in one list. Our model will
# then predict the scores for test data
def trainOneCategory(hashtagTweets, mostCommonWord, profanityWords, negativeWords, positiveWords, emoticons, slangWords):
    features = []
    classes = []
    pun = processPuns(mostCommonWord)
...
def learn_feature_vectors_and_scores(subdir):
    ...
    for i in range(len(dataList)):  # process each category (hashtag) separately
        ...
        print("currently training: " + hashtagTweets[0].hashtag)
        [feats, scores] = trainOneCategory(hashtagTweets, mostCommonWord, wordData[0], wordData[1], wordData[2], wordData[3],
                                slangData[0])
        all_feature_vectors = all_feature_vectors + feats
        all_scores = all_scores + scores

    return[all_feature_vectors, all_scores]

# TRAINING THE MODEL
[train_features, train_scores] = learn_feature_vectors_and_scores("train_data/")  # fit the model on this data
(X, y) = (numpy.array(train_features), numpy.array(train_scores))
from sklearn.svm import SVC
clf = SVC(probability=True)
clf.fit(X, y)

def predictScores(subdir, clf):
    dataList = []  # 2D list, [hashtag][tweet]
    hashtags = []  # list of all hashtags
    rawHashtags = []  # raw hashtags, easier to tokenize in clustering
    subdirectory = subdir
    ...
    for i in range(len(dataList)):  # process each category (hashtag) separately
        hashtagTweets = dataList[i]  # tweets from the same hashtag
        mostCommonWord = getMostCommonWords(hashtagTweets)
        print("currently predicting: " + hashtagTweets[0].hashtag)
        [feats, scores] = trainOneCategory(hashtagTweets, mostCommonWord, wordData[0], wordData[1], wordData[2], wordData[3],
                               slangData[0])
        groundtruths = groundtruths + scores
        predictionsOneTag = clf.predict_proba(numpy.array(feats)) #get prediction confidence scores
        funnyScores = []
        for prediction in predictionsOneTag:
            funnyScores.append(prediction[1])

        # now choose 10 best
        for j in range(0, 10):
            maxIndex = numpy.argmax(funnyScores)
            funnyScores[maxIndex] = -1

        for j in range(len(funnyScores)):
            if funnyScores[j] == -1:
                funnyScores[j] = 1
            else:
                funnyScores[j] = 0

        predictions = predictions + funnyScores
    return[groundtruths, predictions]

#GET PREDICTIONS
[gr, preds] = predictScores("test_data/", clf)

#EVALUATION
numCorrect = 0  #how many we correctly classified as top 10 funniest in their category
numFailed = 0  #how many times we failed to predict top 10
for i in range(len(preds)):
    ...
    if prediction == 1 and groundtruth == 1:
        ...
        numFailed += 1
print("Total classified tweets: " + str(len(preds)))
print("Correctly classified as top 10: " + str(numCorrect))
print("Incorrectly classified as top 10: " + str(numFailed))
